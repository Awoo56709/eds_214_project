...
```{r}
#ensure PRM becomes a sample_id
...
```{r}
#need to split up or group nutrients by site before rolling average. group_split will split into a list of dfs
nutrient_groups <- nutrients %>% group_by(sample_id) %>% group_split()
...
#will need to add this to R and call the function <<<<<<<<<<------------------------------
...
```{r}
#The updated nutrients table that includes the rolling mean based on if values were provided

#will need to add this to the outputs folder <<<<<<<<<<<<----------------------------------
nutrients
```
...
#   #is not na
#   filter(!is.na(value)) %>% 
#   group_by(nutrient, sample_id,sample_date)
# #%>% 
...
#      y = value, 
  ...
  # ) + 
  # 
  # facet_wrap(~ nutrient, scales = "free_y", ncol = 1, strip.position = "left") + #this moves the nutrients to the left
